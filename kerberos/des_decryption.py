...
def test(ciphertext, key1):
    ...
    key = get_key(key1)
    k = int(len(plaintext)/16)
    keybit = Hex2bin(key)
    cipher = ''
    for i in range(0, k):
        ptext = Hex2bin(plaintext[i*16:(i+1)*16])
        cipher += Decryption(ptext, keybit)

    time2 = time.time()
    p = get_plaintext(cipher)

    #print("p = ", p)

    #print("time = ", (time2 - time1)*1000, "ms")
    return p